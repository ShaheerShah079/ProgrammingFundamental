...
struct student search(struct student a[10])
{
	int i,check;
	printf("Enter student id to check their data\n");
	scanf("%d",&check);
	for(i=0;i<10;i++)
	{
		if(a[i].id==check)
		...
		printf("Student blood group is %s\n",a[i].blood);
		printf("Student Programe name is %s\n",a[i].progname);
		printf("Student course register is %s\n",a[i].coursereg);
